/**
 * History Page - Uses Watch History + Video Provider APIs
 */
...
export default async function HistoryPage() {
    ...
    // Get user progress from DB (BFF pattern - only user data in DB)
    ...
    // Fetch drama and episode details from video provider
    const historyItems = await Promise.all(
        progressRecords.map(async (record) => {
            const drama = await videoProvider.getDramaById(record.dramaId)
            const episodes = await videoProvider.getEpisodes(record.dramaId)
            const episode = episodes.find(e => e.id === record.episodeId)
            ...
            return {
                ...
                lastPosition: record.currentTime,
                ...
                episode: {
                    ...
                    thumbnail: episode.thumbnail || drama.bannerUrl,
    ...
    const validItems = historyItems.filter((item): item is NonNullable<typeof item> => item !== null)
    ...
    return (
        <div className="min-h-screen p-6 pt-24 space-y-8">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {validItems.map((item) => {
                    const progress = getProgressPercentage(item.lastPosition, item.duration)
                    return (
                        <Link
                            key={item.id}
